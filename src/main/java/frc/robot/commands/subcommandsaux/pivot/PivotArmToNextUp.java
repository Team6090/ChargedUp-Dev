...
public class PivotArmToNextUp extends CommandBase {
  ...
  public PivotArmToNextUp(PivotSystem pivotSystem, double deg, double allowedNextCommandDeg, boolean autonMode) {
  ...
  public void execute() {
    if (autonMode == true) {
      ...
      if (pos > allowedNextCommandDeg) { // Only when setPos is greater than current pos
        done = true;
      }
    } else {
      done = false;
