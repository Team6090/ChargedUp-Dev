...
public class ArmExtensionToNextOut extends CommandBase {
    ...
    public ArmExtensionToNextOut(TelescopeSystem telescopeSystem, double set, double allowedNextCommandSet, boolean autonMode) {
    ...
    public void execute() {
      if (autonMode == true) {
        ...
        if (currentPos > allowedNextCommandSet) { // Only for extend out
          done = true;
        }
      } else {
        done = false;
