...
public class Drivetrain extends SubsystemBase {
  ...
  public Drivetrain(
    ...
    tabMain.addNumber("Gyroscope Angle", () -> gyroIO.getYaw());
  ...
  //   if (gyroIO.isConnected()) {
  //     return Rotation2d.fromDegrees(gyroIO.getYaw() + this.gyroOffset);
  //   } else {
  ...
  public Rotation2d getGyroscopeRotation() {
    ...
    if (gyroIO.isMagnetometerCalibrated()) {
      // We will only get valid fused headings if the magnetometer is calibrated
      return Rotation2d.fromDegrees(gyroIO.getFusedHeading());
    }

    // We have to invert the angle of the NavX so that rotating the robot counter-clockwise makes
    // the angle increase.
    return Rotation2d.fromDegrees(360.0 - gyroIO.getYaw());
  }
        ...
        if (isFieldRelative) {
          chassisSpeeds =
              ChassisSpeeds.fromFieldRelativeSpeeds(
                  xVelocity, yVelocity, rotationalVelocity, getGyroscopeRotation());
  ...
  public void periodic() {
    ...
    poseEstimator.updateWithTime(
        this.timer.get(), this.getGyroscopeRotation(), swerveModulePositions);
